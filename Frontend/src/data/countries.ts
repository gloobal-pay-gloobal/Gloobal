...
export const TOP_COUNTRIES: Country[] = ([
  ...
  ["New Zealand", "NZ", "+64"],
  ["Iceland", "IS", "+354"],
] as [string, string, string][]).map(([name, iso, dialCode]) => ({ name, iso, dialCode, flag: isoToFlag(iso) }));
...
export const COUNTRY_BY_ISO: Record<string, Country> = Object.fromEntries(
  ALL_COUNTRIES.map((c) => [c.iso, c])
);

// The one search predicate used by every country search box in the app
// (registration's country picker, and the Global Coverage search) so
// ordering/matching behavior is identical everywhere a country is searched.
export function countryMatches(country: Country, rawQuery: string): boolean {
