...
export function logoUrlForDomain(domain, size = 128) {
...
export const TOP5_ORDER_OVERRIDE = {
...
export function buildBanks(countryCode, dial) {
  const all = BANK_NAMES[countryCode] || [];
  const order = TOP5_ORDER_OVERRIDE[countryCode];
  const top5 = order
    ? order.map((name) => all.find((entry) => entry[0] === name)).filter(Boolean)
    : all.slice(0, 5);
...
export const BANKS_BY_COUNTRY = Object.fromEntries(
